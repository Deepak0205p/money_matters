...
// Centralized Progressive Tool & Strategy Unlock Requirements
export const TOOL_UNLOCK_REQUIREMENTS = {
  // Day 1 Tools & Features (Always unlocked)
  'goals': { reqModule: 0, title: 'Day 1 Feature' },
  'chatbot': { reqModule: 0, title: 'Day 1 Feature' },
  'news': { reqModule: 0, title: 'Day 1 Feature' },
  'quiz': { reqModule: 0, title: 'Day 1 Feature' },
  'spin': { reqModule: 0, title: 'Day 1 Feature' },
  'memory': { reqModule: 0, title: 'Day 1 Feature' },
  'word': { reqModule: 0, title: 'Day 1 Feature' },
  'habit': { reqModule: 0, title: 'Day 1 Feature' },
  'badges': { reqModule: 0, title: 'Day 1 Feature' },
  'achievement': { reqModule: 0, title: 'Day 1 Feature' },
  'savings': { reqModule: 0, title: 'Day 1 Feature' },

  // Module-specific Tools (Strictly unlocked via module completion)
  'priority': { reqModule: 2, moduleTitle: 'Module 2: Budgeting In Real Life' },
  'expense': { reqModule: 2, moduleTitle: 'Module 2: Budgeting In Real Life' },
  'sip': { reqModule: 3, moduleTitle: 'Module 3: Saving Strategies' },
  'emergency': { reqModule: 4, moduleTitle: 'Module 4: Emergency Fund' },
  'invest': { reqModule: 8, moduleTitle: 'Module 8: Stock Market & Investments' },
  'age': { reqModule: 5, moduleTitle: 'Module 5: Banking Basics' },
  'health': { reqModule: 5, minModulesCount: 5, moduleTitle: 'Complete Any 5 Modules' },
};

// Check if a tool is unlocked based on user progress
export function isToolUnlocked(toolId, completedModules = []) {
  const info = TOOL_UNLOCK_REQUIREMENTS[toolId];
  if (!info || info.reqModule === 0) return true;

  if (info.minModulesCount) {
    return completedModules.length >= info.minModulesCount;
  }

  return completedModules.includes(info.reqModule);
}

// Get unlock details for a tool
export function getToolUnlockInfo(toolId) {
  return TOOL_UNLOCK_REQUIREMENTS[toolId] || { reqModule: 0, title: 'Unlocked' };
}
