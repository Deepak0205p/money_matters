...
const HABITS = [
  {
    id: 'track',
    label: 'Did you note today\'s expenses?',
    emoji: '💰',
    description: 'Track all expenses today'
  },
  {
    id: 'avoid',
    label: 'Did you avoid unnecessary expenses?',
    emoji: '☕',
    description: 'Skipped an avoidable expense'
  },
  {
    id: 'learn',
    label: 'Did you learn a new financial concept?',
    emoji: '📚',
    description: 'Learned something new about money'
  },
  {
    id: 'save',
    label: 'Did you add money to savings?',
    emoji: '💵',
    description: 'Added money to savings'
  },
  {
    id: 'check',
    label: 'Did you check UPI transactions?',
    emoji: '📱',
...
const FLAME_TIERS = [
  ...
  { min: 14, emoji: '💎', label: 'Habit', color: '#8B5CF6' },
  { min: 7, emoji: '🔥', label: 'Streak', color: '#F97316' },
  { min: 3, emoji: '🌱', label: 'Beginning', color: '#10B981' },
  ...
  useEffect(() => {
    if (allHabitsDone && !perfectDayAwarded) {
      ...
      toast({
        ...
        description: 'All financial habits completed today! Keep it up.'
      });
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Habit Tracker 📅</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll">
          ...
          <div className="p-4 rounded-3xl bg-purple-500/[0.03] border border-purple-500/15 flex gap-3 relative overflow-hidden">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-300 leading-relaxed font-semibold mt-1">
                {!allHabitsDone 
                  ? `Today's checklist is still incomplete! ⏰ Only ${HABITS.length - todayDone.length} more remaining.` 
                  : bestStreak >= 7 
                    ? '7+ day streak! You\'re on the next level. Don\'t skip tomorrow either! 💪' 
                    : 'Perfect day! Keep it up — in 7 days you\'ll unlock the streak badge! 🏆'}
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-3xl bg-white/[0.02] border border-white/[0.04] space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2.5">
              {[
                { id: 'habit-shuruat', days: 3, emoji: '🌱', label: 'Beginning' },
                { id: 'habit-consistent', days: 7, emoji: '🔥', label: 'Streak' },
                { id: 'habit-adaat', days: 14, emoji: '💎', label: 'Habit' },
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Habit Tracker — daily verification resets at 12:00 AM local time
          </p>
        </div>
      </motion.div>
    </div>
