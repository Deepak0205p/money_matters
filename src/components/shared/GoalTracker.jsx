...
const TEMPLATES = [
  ...
  { id: 'phone', label: 'New Phone 📱', emoji: '📱', target: 30000, color: '#8B5CF6', hint: 'Save ₹80 daily. New phone ready in 1 year!' },
  { id: 'trip', label: 'Goa Trip ✈️', emoji: '✈️', target: 20000, color: '#10B981', hint: 'Save ₹60 daily. Fun trip ready in 10 months!' },
  { id: 'emergency', label: 'Emergency Fund 🛡️', emoji: '🛡️', target: 50000, color: '#F59E0B', hint: 'Secure 3 months of survival. Daily ₹140 savings target.' },
  { id: 'laptop', label: 'Work Laptop 💻', emoji: '💻', target: 60000, color: '#06B6D4', hint: 'Save ₹180 daily or earn a small freelance side income.' },
...
export default function GoalTracker({ open, onClose }) {
  ...
  const handleAddGoal = () => {
    ...
    toast({
      ...
      description: "Check your roadmap now and start saving!"
    });
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Dreams to Reality 🎯</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll">
          ...
          {goals.length === 0 ? (
            <div className="text-center py-12 text-zinc-500 text-xs bg-[#0B0E19] border border-white/[0.04] rounded-3xl space-y-2">
              ...
              <p className="font-extrabold text-white">No goals set yet!</p>
              <p className="px-6">Use preset templates to set your first target and start building your safety path.</p>
            </div>
          ) : (
                        ...
                        <div key={g.id} className="p-4.5 rounded-3xl bg-[#0B0E19] border border-white/[0.04] flex flex-col justify-between space-y-4">
                          ...
                          {dailyRate > 0 && (
                            <span className="text-[10px] text-emerald-400 font-semibold block leading-relaxed pt-1 border-t border-white/[0.03]">
                              💡 Save ₹{dailyRate} daily and you'll be done in {days} days!
                            </span>
                          )}
                        </div>
                      );
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Dreams to Reality — Claim +50 Coins on goal achievement
          </p>
        </div>
      </motion.div>
    </div>
