...
const OPTIONS = [
  {
    ...
    tagEmoji: '👨‍🎓',
    description: 'Safe in the bank, but returns are a bit low. Start your first investment here.',
    pros: 'Safe & guaranteed',
    cons: 'Returns may lag behind inflation'
  },
  {
    id: 'sip',
    emoji: '📈',
    ...
    bg: 'bg-amber-400/10',
    minAmount: 500,
    lockIn: 'No lock-in (ELSS: 3 years)',
    tag: 'Long-term growth',
    tagEmoji: '🚀',
    description: 'Start with ₹500/month. Gives the highest returns in the long run.',
  ...
  },
  {
    ...
    bg: 'bg-violet-400/10',
    minAmount: 500000,
    lockIn: 'Long-term (5+ years)',
    tag: 'High ticket size',
    tagEmoji: '💼',
    description: 'Requires big money. Difficult for students right now — save for later.',
    pros: 'Tangible asset, rent income',
    cons: 'Requires large capital, low liquidity'
  },
  {
    id: 'gold',
    emoji: '🥇',
    ...
    tagEmoji: '🛡️',
    description: 'Buy gold ETFs or Sovereign Gold Bonds during festivals — not jewelry!',
  ...
  },
  {
    ...
    tagEmoji: '🎢',
    description: 'Direct company shares. Need knowledge, otherwise money can be lost.',
  ...
  },
  {
    ...
    tag: 'Tax-free return',
    tagEmoji: '🧾',
    description: 'Govt scheme — tax-free interest. Best for long-term retirement.',
  ...
  },
  {
    ...
    tagEmoji: '👴',
    description: 'Extra tax savings of ₹50k. Solid for retirement planning.',
...
export default function InvestmentComparison({ open, onClose }) {
  ...
  const toggleSelection = (id) => {
    setSelected(prev => {
      ...
      if (prev.length >= 3) {
        toast({
          title: 'Select max 3 items! 🎯',
          description: 'A maximum of 3 investments can be compared at once.'
        });
        return prev;
      }
      return [...prev, id];
  ...
  const handleCompare = () => {
    if (selected.length < 2) {
      toast({
        ...
        description: 'Select a minimum of two schemes for comparison.'
      });
      return;
    }
  ...
  const aiSuggestion = useMemo(() => {
    if (selected.length === 0) {
      return 'Select 2-3 options from above, then tap "Compare" — you\'ll get a full breakdown! 👇';
    }
    if (selected.includes('sip') && selected.includes('ppf')) {
      return 'SIP + PPF combo is the best — growth + tax saving + safety! 🚀 This would be perfect for your age.';
    }
    if (selected.includes('fd')) {
      return 'FD is safe but loses to inflation. Add SIP too for long-term growth.';
    }
    if (selected.includes('stocks') && selected.length === 1) {
      return 'Stocks alone are risky. A 70-30 mix of SIP + Stocks would be better. Knowledge is essential!';
    }
    if (selected.includes('gold')) {
      return 'Keep gold at 10-15% of your portfolio. Don\'t make it your entire investment.';
    }
    return 'You\'re absolutely on the right track! Tap "Start Investing" and open an account on Groww/Zerodha.';
  }, [selected]);
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Big Decision 🔍</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll">
          ...
          <div className="flex items-center gap-2.5 p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.04]">
            ...
            <p className="text-[11px] text-zinc-400 font-semibold leading-relaxed">
              Let's play! Tap any 2 or 3 schemes and check the side-by-side comparison matrix.
            </p>
          </div>
            ...
            <button
              onClick={() => {
                ...
                toast({
                  ...
                  description: 'Moving towards success. +5 Coins added!'
                });
              }}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Big Decision — rates are approximate based on past decade performance averages
          </p>
        </div>
      </motion.div>
    </div>
