...
const QUESTIONS = [
  {
    ...
    subtitle: 'Does it cover 3-6 months of expenses?',
    heroEmoji: '🛡️',
    options: [
      { text: 'No fund', score: 0, emoji: '😰' },
      { text: '1 month\'s worth', score: 4, emoji: '😐' },
      { text: '3 months\' worth', score: 8, emoji: '😊' },
      { text: '6+ months\' worth', score: 12, emoji: '🤩' }
    ]
  },
  {
    id: 'budgeting',
    question: 'Do you make a monthly budget?',
    subtitle: 'Do you plan your expenses or spend randomly?',
    heroEmoji: '📝',
    options: [
      { text: 'Budget? What\'s that!', score: 0, emoji: '🙈' },
      { text: 'In my head, not on paper', score: 4, emoji: '🤔' },
      { text: 'Yes, I make a basic budget', score: 8, emoji: '✅' },
      { text: 'I track in detail', score: 12, emoji: '📊' }
    ]
  },
  {
    id: 'savings',
    question: 'How much do you save monthly?',
    subtitle: 'What percentage of income do you save?',
    heroEmoji: '💰',
    options: [
      { text: 'Can\'t save anything', score: 0, emoji: '💸' },
      { text: 'Less than 10%', score: 4, emoji: '🪙' },
      { text: 'I save 10-20%', score: 8, emoji: '🏦' },
      { text: 'More than 20%!', score: 12, emoji: '🚀' }
    ]
  },
  {
    id: 'debt',
    question: 'How do you pay your credit card bill?',
    subtitle: 'Do you pay the full bill or minimum?',
    heroEmoji: '💳',
    options: [
      { text: 'I pay the minimum', score: 0, emoji: '😱' },
      { text: 'Sometimes full, sometimes partial', score: 4, emoji: '😬' },
      { text: 'I always pay in full', score: 12, emoji: '💪' },
      { text: 'I don\'t use a credit card', score: 8, emoji: '🚫' }
    ]
  },
  {
    id: 'investment',
    question: 'Have you invested anywhere?',
    subtitle: 'SIP, FD, mutual fund, stocks?',
    heroEmoji: '📈',
    options: [
      { text: 'No, not yet', score: 0, emoji: '😴' },
      { text: 'In FD/Savings account', score: 4, emoji: '🏦' },
      { text: 'Yes, mutual fund/SIP', score: 12, emoji: '🎯' },
  ...
  },
  {
    ...
    subtitle: 'Both health and life insurance?',
    heroEmoji: '🛡️',
    options: [
      { text: 'No insurance', score: 0, emoji: '🚨' },
      { text: 'Only company\'s', score: 4, emoji: '🤷' },
      { text: 'Have health insurance', score: 8, emoji: '👍' },
      { text: 'Have both Health + Life', score: 12, emoji: '✅' }
    ]
  },
  {
    id: 'learning',
    question: 'How much do you learn about finance?',
    subtitle: 'Do you watch videos, read articles, books?',
    heroEmoji: '📚',
    options: [
      { text: 'Never studied', score: 0, emoji: '🙈' },
      { text: 'Sometimes watch YouTube', score: 4, emoji: '📺' },
      { text: 'Regularly follow content', score: 8, emoji: '📖' },
      { text: 'Read + apply', score: 12, emoji: '🎓' }
    ]
  }
];
...
function getGrade(pct) {
  if (pct >= 75) return { label: 'Fit', emoji: '💪', color: '#10B981', tip: 'You\'re a financial ninja! Keep it up!' };
  if (pct >= 50) return { label: 'Average', emoji: '🤔', color: '#F59E0B', tip: 'You\'re okay, but improvement is needed!' };
  return { label: 'Critical', emoji: '🏥', color: '#EF4444', tip: 'Don\'t worry, start now!' };
}

function getRecommendations(answers) {
  const tips = [];
  if ((answers.emergency ?? 0) < 8) tips.push('🛡️ Build an emergency fund first — cover 3 months of expenses.');
  if ((answers.budgeting ?? 0) < 8) tips.push('📝 Make a monthly budget — follow the 50-30-20 rule.');
  if ((answers.savings ?? 0) < 8) tips.push('💰 Save at least 20% of income — automate it!');
  if ((answers.debt ?? 0) < 8) tips.push('💳 Always pay credit card in full — minimum payment is a trap!');
  if ((answers.investment ?? 0) < 8) tips.push('📈 Start a SIP — even ₹500/month can work compounding magic!');
  if ((answers.insurance ?? 0) < 8) tips.push('🏥 Health insurance is essential — medical emergencies eat up savings!');
  if ((answers.learning ?? 0) < 8) tips.push('📚 Increase financial literacy — read 1 video/article every week!');
  if (tips.length === 0) tips.push('🌟 You\'re already on track! Set new goals and keep growing!');
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Money Doctor 🏥</h2>
              ...
              >
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-zinc-400 max-w-sm mx-auto leading-relaxed">
                    Answer 7 simple questions and get a dynamic fitness score check with AI recommendations!
                  </p>
                </div>
                  ...
                  <button
                    onClick={() => {
                      if (typeof navigator !== 'undefined' && navigator.share) {
                        navigator.share({
                          title: 'Money Doctor Report Card',
                          text: `My Financial Wellness Health Score: ${finalCheckupReport.score}/100 (${finalGrade.label} ${finalGrade.emoji})! Take a quick checkup too!`
                        }).catch(() => {});
                      }
                    }}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Money Doctor — report results are strictly for learning and guidance purposes
          </p>
        </div>
      </motion.div>
    </div>
