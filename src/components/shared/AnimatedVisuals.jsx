...
export function BarterAnimation({ color }) {
  return (
    <div className="relative w-full h-48 flex items-center justify-center overflow-hidden rounded-xl bg-slate-50 border border-slate-200">
      {/* Background glow */}
      <motion.div 
...
export function UPINetworkAnimation({ color }) {
  ...
  return (
    <div className="relative w-full h-48 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-200 overflow-hidden">
...
export function BudgetPieAnimation({ color }) {
  return (
    <div className="relative w-full h-56 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-200 overflow-hidden">
...
export function NeedsWantsScaleAnimation({ color }) {
  return (
    <div className="relative w-full h-48 flex flex-col items-center justify-end pb-8 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden">
      {/* Stand */}
      <div className="w-1.5 h-20 bg-slate-400 rounded-t-md relative z-10" />
      <div className="w-16 h-2 bg-slate-400 rounded-md absolute bottom-8 z-10" />
      
      {/* Beam */}
      <motion.div 
        className="w-48 h-1.5 bg-slate-500 rounded-full absolute bottom-28 origin-bottom"
        animate={{ rotate: [-10, 5, -10] }}
        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      >
        {/* Left hanging line (Needs) */}
        <div className="absolute -left-0.5 top-0 w-0.5 h-12 bg-slate-400/50" />
        {/* Right hanging line (Wants) */}
        <div className="absolute -right-0.5 top-0 w-0.5 h-12 bg-slate-400/50" />
      </motion.div>
...
export function BufferTankAnimation({ color }) {
  return (
    <div className="relative w-full h-56 flex flex-col items-center justify-center rounded-xl bg-slate-50 border border-slate-200 overflow-hidden">
      
      {/* Faucet (Income) */}
      <div className="absolute top-4 w-6 h-6 border-l-4 border-t-4 border-slate-400 rounded-tl-lg" />
      ...
      {/* Tank container */}
      <div className="w-32 h-32 border-4 border-slate-400 rounded-b-xl rounded-t-sm relative overflow-hidden mt-8 shadow-[0_0_20px_rgba(245,158,11,0.2)]">
        {/* Buffer liquid level */}
        <motion.div 
      ...
      {/* Drain pipe (Expenses) */}
      <div className="w-2 h-8 bg-slate-400 absolute bottom-0" />
      <motion.div 
