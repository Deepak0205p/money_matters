...
export default function EmergencyFundCalculator({ open, onClose }) {
  ...
  const tier = useMemo(() => {
    if (protectionPct >= 100) return {
      ...
      desc: 'Fantastic! Your Safety Shield is ready. Stay worry-free.'
    };
    if (protectionPct >= 60) return {
      key: 'strong', label: 'Strong Shield 🛡️', color: '#06B6D4', ring: 'rgba(6,182,212,0.4)',
      desc: 'You\'re on a good track. You\'ll be safe in an emergency.'
    };
    if (protectionPct >= 20) return {
      key: 'half', label: 'Partial Shield ⚠️', color: '#F59E0B', ring: 'rgba(245,158,11,0.3)',
      desc: 'Some safety is there, but increase your fund for a heavy financial storm.'
    };
    return {
      key: 'broken', label: 'Broken Shield 🚨', color: '#EF4444', ring: 'rgba(239,68,68,0.3)',
      desc: 'Financial risk is high without a backup! Increase your savings.'
    };
  }, [protectionPct]);
  ...
  useEffect(() => {
    if (protectionPct >= 100 && !targetReached) {
      ...
      toast({
        ...
        description: "You've unlocked the milestone badge."
      });
  ...
  const handleSavePlan = () => {
    ...
    toast({
      ...
      description: "Your custom Emergency goal value has been set."
    });
  };
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Safety Shield 🛡️</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll">
          ...
          <div className="bg-amber-500/5 border border-amber-500/15 rounded-3xl p-4 flex items-start gap-3">
            ...
            <div className="space-y-1">
              <h4 className="text-xs font-black text-amber-300">Stat Fact: 80% of Indians don't have an emergency fund.</h4>
              <p className="text-[10px] text-zinc-400 leading-relaxed">
                Emergency funds should be kept in liquid assets (Savings Account or Liquid Funds) where you can withdraw instantly, not in FDs or Stocks where there's heavy lock-in/losses.
              </p>
            </div>
          </div>
        </div>
