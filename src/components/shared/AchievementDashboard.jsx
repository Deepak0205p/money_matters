...
const BADGES = [
  ...
  { id: 'three-modules', name: 'Pilgrim', description: 'Completed 3 modules!', Icon: MapPin, color: '#22c55e' },
  { id: 'six-modules', name: 'Finance Warrior', description: 'Completed 6 modules!', Icon: Sword, color: '#3b82f6' },
  { id: 'all-modules', name: 'Finance Guru', description: 'All 11 modules completed!', Icon: Crown, color: '#f59e0b' },
  { id: 'perfect-quiz', name: 'Quiz Master', description: 'Scored 100% in any quiz!', Icon: Award, color: '#a855f7' },
  ...
  { id: 'debt-escape', name: 'Debt Free', description: 'Opened all Debt Trap doors!', Icon: DoorOpen, color: '#6366f1' },
  { id: 'early-bird', name: 'Early Bird', description: 'Started investing at age 20!', Icon: Bird, color: '#06b6d4' }
];
...
function getMotivationalMessage(completed, score) {
  if (completed === 0) return 'Start your journey — every great journey begins with a single step! 💪';
  if (completed <= 3) return 'Good start! A little more effort and your financial understanding will get even stronger! 🔥';
  if (completed <= 6) return 'You\'re halfway there — don\'t turn back, keep moving forward! 🚀';
  if (completed <= 9) return 'Excellent progress! The finish line is in sight! 🏆';
  if (completed <= 10) return 'Just one more module — you\'re almost a Finance Guru! 🎯';
  return 'You\'re a true Finance Guru! Now share your knowledge and teach others! 👑';
}
...
function ProgressForecast({ modulesCompleted, moduleCompletionDates }) {
  const forecast = useMemo(() => {
    if (modulesCompleted === 0) {
      return 'Once you complete the first module, we can calculate your pace! Get started! 🚀';
    }
    if (modulesCompleted >= 11) {
      return 'You\'ve completed all modules! 🎉 You\'re now a guru — teach others!';
    }

    const dates = Object.values(moduleCompletionDates || {}).sort();
    if (dates.length < 2) {
      const remaining = 11 - modulesCompleted;
      return `First module done! If you do one module daily, you'll finish in ~${remaining} days. Keep at it! 💪`;
    }
    ...
    return `At your current pace, all modules will be completed by ${formattedDate}! (${estimatedDaysLeft} days remaining) 📅`;
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Achievement Board 📊</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll">
              ...
              <div className="text-center sm:text-left flex-1 space-y-1">
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 pt-2">
                  ...
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-black">
                     <Calendar size={12} /> {daysActive} Days
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Active next steps */}
          <DashboardSection>
            <SectionTitle icon={ArrowRight} title="What to Do Next? (Next Actions)" subtitle="AI recommended path forwards" />
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {nextSteps.length > 0 ? (
                ...
                <div className="col-span-3 text-center py-6 text-zinc-500 font-bold uppercase tracking-wider">
                   🎉 You've achieved all curriculum steps! Outstanding job.
                </div>
              )}
            </div>
          </DashboardSection>
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Achievement Board — achievements sync across local storage and active sessions
          </p>
        </div>
      </motion.div>
    </div>
