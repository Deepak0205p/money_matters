...
function AnimatedFlowchart({ data, color = '#3B82F6' }) {
  const nodes = data?.nodes || [];
  const edges = data?.edges || [];
  return (
    <div className="relative w-full overflow-x-auto py-4 px-2 my-3 rounded-2xl bg-slate-50 border border-slate-200">
      <div className="relative min-w-[320px] h-48 flex items-center justify-center">
        ...
        {nodes.map((node, i) => (
          <motion.div
            ...
            className="flex items-center justify-center rounded-xl bg-slate-800 border text-xs font-bold text-white shadow-md transition-transform hover:scale-105"
          >
            {node.label}
          </motion.div>
        ))}
...
function AnimatedPieChart({ data, color = '#10B981' }) {
  const slices = data?.slices || [{ label: 'Needs', value: 50, color: '#10B981' }, { label: 'Wants', value: 30, color: '#F59E0B' }, { label: 'Savings', value: 20, color: '#8B5CF6' }];
  return (
    <div className="flex flex-col sm:flex-row items-center justify-around gap-4 p-4 my-3 rounded-2xl bg-slate-50 border border-slate-200">
      <div className="relative w-28 h-28 flex items-center justify-center">
        ...
        <span className="absolute text-xs font-bold text-slate-700">50/30/20</span>
      </div>
...
function AnimatedBarChart({ data, color = '#3B82F6' }) {
  const items = data?.items || [];
  return (
    <div className="p-4 my-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-2.5">
      {items.map((it, i) => (
...
function AnimatedDecisionTree({ data, color = '#8B5CF6' }) {
  return (
    <div className="p-4 my-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500">Decision Flow</h4>
      <div className="flex items-center gap-2 text-xs text-slate-700">
        <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 font-semibold">{data?.root || 'Decision'}</span>
        <ArrowRight size={14} className="text-slate-500" />
        <span className="px-2.5 py-1 rounded-lg bg-indigo-500/10 border border-indigo-500/20 font-semibold">{data?.branch || 'Outcome'}</span>
      </div>
    </div>
  );
}

function AnimatedTimeline({ data, color = '#F59E0B' }) {
  return (
    <div className="p-4 my-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
      <div className="text-xs font-bold text-amber-600">{data?.title || 'Timeline'}</div>
      <p className="text-xs text-slate-600 leading-relaxed">{data?.description || ''}</p>
    </div>
  );
}

function AnimatedComparisonCard({ data, color = '#10B981' }) {
  return (
    <div className="grid grid-cols-2 gap-3 p-4 my-3 rounded-2xl bg-slate-50 border border-slate-200 text-xs">
      <div className="p-2.5 rounded-xl bg-white border border-slate-200">
        <div className="font-bold text-slate-700">{data?.leftTitle || 'Option A'}</div>
        <p className="text-slate-500 mt-1">{data?.leftDesc || ''}</p>
      </div>
      <div className="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
        <div className="font-bold text-emerald-600">{data?.rightTitle || 'Option B'}</div>
        <p className="text-emerald-700 mt-1">{data?.rightDesc || ''}</p>
      </div>
    </div>
  );
}

function AnimatedStatHighlight({ data, color = '#10B981' }) {
  return (
    <div className="p-4 my-3 rounded-2xl bg-gradient-to-r from-emerald-50 to-transparent border border-emerald-200 flex items-center gap-3">
      <div className="text-2xl font-black text-emerald-600 tabular-nums">{data?.stat || '₹0'}</div>
      <div className="text-xs text-slate-600 leading-tight">{data?.text || ''}</div>
    </div>
  );
}
...
function parseInline(text, color) {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (m[1]) {
      ...
      className: "bg-slate-100 text-emerald-600 px-1 py-0.5 rounded text-[12px] font-mono",
      children: m[3]
  ...
  while (i < lines.length) {
      ...
      if (match) {
        ...
        blocks.push(/*#__PURE__*/_jsxs(motion.div, {
          ...
          children: [/*#__PURE__*/_jsx("span", {
            className: "text-[10px] font-bold text-slate-400 mb-1 px-2 uppercase tracking-wider",
            children: name
          }), /*#__PURE__*/_jsx("div", {
            className: `max-w-[85%] px-4 py-3 rounded-2xl text-[15.5px] sm:text-[17.5px] font-semibold leading-relaxed shadow-sm ${isBhaiya ? 'rounded-tr-none' : 'bg-slate-100 border border-slate-200 text-slate-700 rounded-tl-none'}`,
            style: isBhaiya ? {
    ...
    if (line.includes('🚨') && (line.includes('MISSION') || line.includes('ALERT'))) {
      ...
      blocks.push(/*#__PURE__*/_jsxs(motion.div, {
        ...
        className: "my-6 p-5 rounded-2xl border-2 border-dashed relative overflow-hidden bg-slate-50",
        style: {
          ...
          children: missionLines.map((mLine, mi) => {
            ...
            if (isCheckbox) {
              return /*#__PURE__*/_jsxs("div", {
                ...
                children: [/*#__PURE__*/_jsx("div", {
                  className: `mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-all ${checkedItems[id] ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 bg-white group-hover:border-slate-400'}`,
                  children: checkedItems[id] && /*#__PURE__*/_jsx(CheckCircle2, {
                    size: 12,
                    className: "text-white"
                  })
                }), /*#__PURE__*/_jsx("span", {
                  className: `text-[13px] leading-snug transition-all ${checkedItems[id] ? 'text-slate-400 line-through' : 'text-slate-700'}`,
    ...
    if (line.trim().startsWith('|')) {
      ...
      if (rowLines.length > 0) {
        ...
        blocks.push(/*#__PURE__*/_jsx("div", {
          className: "overflow-x-auto rounded-xl border border-slate-200 mt-3 mb-2",
          children: /*#__PURE__*/_jsx("table", {
            className: "w-full text-[12px] border-collapse",
            children: /*#__PURE__*/_jsx("tbody", {
              children: rows.map((row, ri) => /*#__PURE__*/_jsx("tr", {
                className: ri === 0 && isHeader ? 'border-b border-slate-200' : ri % 2 === 0 ? 'bg-slate-50' : '',
                children: row.map((cell, ci) => ri === 0 && isHeader ? /*#__PURE__*/_jsx("th", {
                  ...
                  className: `px-3 py-2 text-slate-600 ${ci === 0 ? 'font-medium' : ''}`,
                  children: parseInline(cell, color)
                }, ci))
              }, ri))
            })
    ...
    if (line.trim().match(/^[-•*✅❌]\s/)) {
      ...
      blocks.push(/*#__PURE__*/_jsx("ul", {
        ...
        children: items.map((item, idx) => /*#__PURE__*/_jsxs("li", {
          className: "flex gap-2 text-slate-700 text-[15.5px] sm:text-[17.5px] font-semibold leading-relaxed",
          children: [/*#__PURE__*/_jsx("span", {
    ...
    if (line.trim().match(/^\d+[.)] /)) {
      ...
      blocks.push(/*#__PURE__*/_jsx("ol", {
        ...
        children: items.map((item, idx) => /*#__PURE__*/_jsxs("li", {
          className: "flex gap-2.5 text-slate-700 text-[15.5px] sm:text-[17.5px] font-semibold leading-relaxed",
          children: [/*#__PURE__*/_jsx("span", {
    ...
    if (line.includes('↓') || line.includes('→') || line.includes('←') || line.includes('↑')) {
      blocks.push(/*#__PURE__*/_jsx("p", {
        className: "text-slate-400 text-[12px] leading-relaxed font-mono pl-2 my-0.5",
  ...
  return /*#__PURE__*/_jsx("div", {
    ...
    children: Array.from({
      ...
      return /*#__PURE__*/_jsx("div", {
        ...
        style: {
          backgroundColor: isCompleted ? color : 'rgba(0,0,0,0.08)',
    ...
    children: [/*#__PURE__*/_jsx("h4", {
      ...
      children: data.options.map((option, i) => {
        ...
        return /*#__PURE__*/_jsxs(motion.button, {
          ...
          className: `w-full p-3 rounded-xl border text-left text-sm transition-all flex items-center justify-between ${bgColor} ${borderColor} ${selected === null ? 'hover:bg-slate-100' : ''}`,
          children: [/*#__PURE__*/_jsx("span", {
            className: isSelected || selected !== null && isCorrect ? 'text-slate-900 font-medium' : 'text-slate-600',
            children: option
...
export function InteractiveCalculatorViewer({
  ...
  return /*#__PURE__*/_jsxs("div", {
    className: "mt-6 p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-6",
    children: [/*#__PURE__*/_jsx("div", {
      className: "space-y-5",
  ...
  return /*#__PURE__*/_jsxs("div", {
    ...
    children: [/*#__PURE__*/_jsxs("h4", {
      ...
      children: data.choices.map((choice, i) => /*#__PURE__*/_jsxs(motion.button, {
        ...
        className: `p-4 rounded-xl border text-left transition-all ${selected === i ? 'bg-slate-100 border-slate-300' : 'bg-white border-slate-200 hover:bg-slate-50'}`,
        children: [/*#__PURE__*/_jsx("div", {
          ...
          children: selected === i && /*#__PURE__*/_jsxs(motion.div, {
            ...
            className: "text-[12px] text-slate-500 leading-relaxed mt-2 pt-2 border-t border-slate-200",
            children: [/*#__PURE__*/_jsx("span", {
