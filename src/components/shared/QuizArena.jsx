...
export default function QuizArena({ open, onClose }) {
  ...
  const handleAnswer = useCallback((optIdx) => {
    ...
    if (correct) {
      ...
      setToastInfo({
        type: 'correct',
        message: newCombo > 1 ? `${newCombo}x Streak! 🔥` : 'Correct Answer!',
        coins: coinsWon
      });
    } else {
      setCombo(0);
      setToastInfo({
        type: 'wrong',
        message: optIdx === -1 ? '⏰ Time Up!' : 'Wrong Answer!'
      });
    }
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
                  <h2 className="text-base font-black text-white">Knowledge Arena 🧠</h2>
            ...
            {stage === 'categories' && (
              ...
              >
                <div className="space-y-2">
                  ...
                  <h3 className="text-xl font-black text-white">Select a Category!</h3>
                  <p className="text-xs text-zinc-400 max-w-md mx-auto leading-relaxed">
                    Each quiz has 10 questions. Build a combo streak and earn bonus coins!
                  </p>
                </div>
                ...
                <AnimatePresence>
                  {showExplanation && (
                    ...
                    >
                      ...
                      >
                        {isLastQ ? 'See Score Summary' : 'Move to Next Question'} <ChevronRight size={14} />
                      </button>
                    </motion.div>
                  )}
                </AnimatePresence>
                  ...
                  <button
                    onClick={() => {
                      if (typeof navigator !== 'undefined' && navigator.share) {
                        navigator.share({
                          title: 'Knowledge Arena Score Card',
                          text: `I played the ${category.label} quiz in Knowledge Arena and scored ${score} points with ${accuracyPct}% accuracy! You should try it too!`
                        }).catch(() => {});
                      } else {
                        toast({
                          title: 'Score Card Link copied! 🔗',
                          description: 'Share your scores with friends.'
                        });
                      }
                    }}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Knowledge Arena Quiz — answers are verified using live course curriculum
          </p>
        </div>
      </motion.div>
    </div>
