...
const TIPS = [
  'In SIP, consistency beats timing. Invest regularly!',
  'Emergency fund = 6 months of expenses. Build this first!',
  'Always pay credit card in full — minimum payment = trap!',
  '50-30-20 rule: Needs 50%, Wants 30%, Savings 20%.',
  'Insurance is essential — medical emergency = savings killer!',
  'Mutual funds beat FD — better returns in the long term.',
  'Avoid lifestyle inflation — when income rises, don\'t raise spending.',
  'PPF and ELSS are the best options for tax saving.'
];
      ...
      >
        ...
        <div className="shrink-0 px-6 py-4 border-b border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">Lucky Wheel 🎡</h2>
              <p className="text-[10px] text-zinc-400">One free spin daily — win coins and badges!</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
          ...
          <div className="text-center">
            {canSpin ? (
              ...
              <div className="px-6 py-4 bg-white/5 border border-amber-500/20 rounded-2xl inline-flex items-center gap-3 text-left">
                ...
                <div>
                  <p className="text-xs font-black text-amber-400 uppercase tracking-wider">Next Spin Ready Tomorrow!</p>
          ...
          <AnimatePresence>
            {result && (
              ...
              >
                ...
                {result.type === 'badge' && (
                  <p className="text-xs font-black text-purple-400">New badge added to Achievement Gallery! 🏆</p>
                )}
                {result.type === 'shield' && (
                  <p className="text-xs font-black text-cyan-400">Streak Shield active! Daily checklist skipped shield 🛡️</p>
                )}
                {result.type === 'retry' && (
                  <p className="text-xs font-black text-zinc-500">No worries, consistency is the key! 💪</p>
                )}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0C1021]/80 backdrop-blur-md flex items-center justify-center text-center">
          <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">
            Lucky Wheel — spins are restricted to 1 spin per user account daily
          </p>
        </div>
      </motion.div>
    </div>
