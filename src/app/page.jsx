...
function FloatingSymbols() {
  const symbols = useMemo(() => [
    ...
    { Icon: TrendingUp, color: '#00F29D', size: 32, top: '45%', left: '85%', delay: 2, dur: 14 },
    { Icon: Zap, color: '#6366F1', size: 28, top: '75%', left: '15%', delay: 4, dur: 10 },
    { Icon: Award, color: '#FFB800', size: 20, top: '25%', left: '70%', delay: 1, dur: 13 },
    { Icon: BookOpen, color: '#00F29D', size: 22, top: '60%', left: '80%', delay: 3, dur: 11 },
...
function Background2D() {
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#F8FAFC]">
      <FloatingSymbols />
      <LightStreaks />
      <div className="absolute -top-[20%] -left-[20%] w-[70%] h-[70%] rounded-full bg-emerald-500/[0.04] blur-[150px]" />
      <div className="absolute -bottom-[20%] -right-[20%] w-[70%] h-[70%] rounded-full bg-emerald-500/[0.02] blur-[150px]" />
      <div
          ...
          return (
            <div key={idx} className="rounded-2xl bg-white border border-slate-200/80 shadow-sm overflow-hidden transition-all duration-300">
              <button
                ...
                className="w-full text-left p-5 flex items-center justify-between text-base font-semibold text-slate-900 hover:text-emerald-600 transition-colors cursor-pointer"
              >
                <span>{faq.q}</span>
                <ChevronDown
                  size={18}
                  className={`text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-emerald-600' : ''}`}
                />
              </button>
              <AnimatePresence>
                {isOpen && (
