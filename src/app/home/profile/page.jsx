    ...
    <div className="space-y-6 max-w-5xl mx-auto text-left pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200 p-6 rounded-3xl relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-br from-emerald-500/20 to-indigo-500/20" />
            
            <div className="relative z-10 flex flex-col items-center mt-8">
              <div className="relative group">
                <div className="w-28 h-28 rounded-full bg-slate-100 border-4 border-slate-200 shadow-xl flex items-center justify-center text-5xl overflow-hidden relative">
                  {user?.photoURL ? (
              ...
              <h2 className="text-2xl font-black text-slate-900 mt-5 mb-1 text-center">
              ...
              <button 
                ...
                className="w-full bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-900 font-semibold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-3xl space-y-4 shadow-xl">
             <div className="flex justify-between items-center py-2 border-b border-slate-200">
                ...
                <span className="text-slate-900 text-sm truncate max-w-[150px]">{user?.email || 'N/A'}</span>
             </div>
             <div className="flex justify-between items-center py-2 border-b border-slate-200">
                <span className="text-slate-500 text-sm font-medium">Status</span>
                <span className="text-slate-900 text-sm capitalize">{formData.status || 'Not Set'}</span>
             </div>
             <div className="flex justify-between items-center py-2">
                <span className="text-slate-500 text-sm font-medium">City</span>
                <span className="text-slate-900 text-sm">{formData.city || 'Not Set'}</span>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Main Stats Grid */}
          <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={pill.label} className="bg-slate-100 border border-slate-200 rounded-2xl p-4 text-center shadow-lg relative overflow-hidden group hover:border-slate-200 transition-colors">
                  ...
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-1.5 block relative z-10">{pill.label}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-2xl border transition-all ${
                      isUnlocked
                        ? 'bg-gradient-to-b from-slate-100 to-transparent border-slate-200 shadow-lg hover:bg-slate-100'
                        : 'bg-slate-50 border-slate-100 opacity-40 grayscale'
                    }`}
                    title={badge.description}
                  >
                    <div className="text-3xl mb-2 drop-shadow-md">{badge.emoji}</div>
                    <span className="text-[10px] font-bold text-slate-600 block truncate w-full px-1">{badge.name}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-100">
                ...
                <button onClick={() => setIsEditing(false)} className="text-slate-500 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 p-2 rounded-full transition-colors">
                  <X size={18} />
                </button>
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-100 border border-slate-200 focus:border-emerald-500/50 rounded-xl py-3 pl-10 pr-4 text-slate-900 text-sm outline-none transition-all focus:shadow-[0_0_0_1px_rgba(16,185,129,0.2)]"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative opacity-60">
                    ...
                    <input 
                      ...
                      disabled
                      className="w-full bg-slate-100 border border-slate-200 rounded-xl py-3 pl-10 pr-4 text-slate-500 text-sm outline-none cursor-not-allowed"
                    />
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Current Status</label>
                  <div className="relative">
                    <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-500" />
                    <select
                      value={formData.status}
                      onChange={(e) => setFormData({...formData, status: e.target.value})}
                      className="w-full bg-slate-100 border border-slate-200 focus:border-emerald-500/50 rounded-xl py-3 pl-10 pr-4 text-slate-900 text-sm outline-none transition-all focus:shadow-[0_0_0_1px_rgba(16,185,129,0.2)] appearance-none"
                    >
      ...
      <AnimatePresence>
        {successMsg && (
          <motion.div 
            ...
            className="fixed bottom-6 right-6 z-50 bg-slate-100 border border-emerald-500/30 text-slate-900 px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3"
          >
