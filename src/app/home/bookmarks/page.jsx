  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-xl max-h-[88vh] overflow-hidden rounded-3xl border border-slate-200 shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 shrink-0">
          ...
          <button
            onClick={onClose}
            className="size-8 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition-all shrink-0"
          >
            <X className="size-4" />
          </button>
        </div>
            ...
            >
              ...
              {item.card.example && (
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-4">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">{item.card.example}</p>
                </div>
              )}
              ...
              {item.card.tip && (
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">{item.card.tip}</p>
                </div>
              )}
            </motion.div>
          </AnimatePresence>
        ...
        {/* Navigation footer */}
        <div className="px-5 py-3 border-t border-slate-200 flex items-center justify-between shrink-0 bg-black/20">
          <button
            disabled={idx === 0}
            onClick={() => setIdx((p) => p - 1)}
            className="px-4 py-1.5 rounded-xl text-xs font-semibold bg-slate-50 border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
            ← Prev
          </button>
          <span className="text-[11px] text-slate-600">{idx + 1} / {items.length}</span>
          <button
...
function BookmarkCard({ item, onOpen, onRemove, view }) {
  ...
  if (view === 'list') {
    return (
      <motion.div
        ...
        whileHover={{ x: 3 }}
        className="group flex items-center gap-4 bg-slate-50 hover:bg-slate-100 border border-slate-200 hover:border-slate-300 rounded-2xl px-4 py-3.5 transition-all cursor-pointer"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-semibold text-slate-900 truncate">{item.card.title}</p>
          {item.card.topicTitle && (
            <p className="text-[11px] text-slate-600 truncate mt-0.5">{item.card.topicTitle}</p>
          )}
        </div>
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              className="p-1.5 rounded-lg bg-slate-100 text-slate-500 hover:text-slate-900 transition-all"
            >
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-2xl p-4 flex flex-col gap-3 cursor-pointer transition-all overflow-hidden"
      onClick={() => onOpen(item.id)}
    >
      {/* Top color accent */}
      ...
      {/* Card title */}
      <h3 className="text-sm font-bold text-slate-900 leading-snug line-clamp-2">{item.card.title}</h3>
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="font-display text-xl font-extrabold text-slate-900 tracking-tight">My Bookmarks</h1>
          </div>
      ...
      {items.length === 0 ? (
        ...
        <motion.div
          ...
          className="flex flex-col items-center justify-center py-20 text-center bg-slate-50 border border-slate-200 rounded-3xl"
        >
          ...
          <h2 className="font-display text-lg font-bold text-slate-900 mb-2">No bookmarks yet</h2>
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500/40 focus:outline-none rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-600 transition-all"
              />
              {search && (
                <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-slate-600 transition-colors">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-600 hover:bg-slate-100 transition-all"
              >
                <Filter className="size-3.5 text-slate-500" />
                {filterModule === 'all' ? 'All Modules' : modules.find((m) => String(m.id) === filterModule)?.title.split(' ')[0]}
                <ChevronDown className={`size-3.5 text-slate-600 transition-transform ${showFilterMenu ? 'rotate-180' : ''}`} />
              </button>
              ...
              <AnimatePresence>
                {showFilterMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-56 z-50 bg-white border border-slate-200 rounded-2xl shadow-2xl p-2 space-y-0.5"
                  >
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-600 focus:outline-none focus:border-emerald-500/40 transition-all cursor-pointer"
            >
            ...
            {/* View toggle */}
            <div className="flex items-center gap-0 bg-slate-50 border border-slate-200 rounded-xl p-1">
              <button
                onClick={() => setView('grid')}
                className={`p-1.5 rounded-lg transition-all ${view === 'grid' ? 'bg-emerald-500/15 text-emerald-400' : 'text-slate-600 hover:text-slate-600'}`}
              >
                <Grid3X3 className="size-3.5" />
              </button>
              <button
                onClick={() => setView('list')}
                className={`p-1.5 rounded-lg transition-all ${view === 'list' ? 'bg-emerald-500/15 text-emerald-400' : 'text-slate-600 hover:text-slate-600'}`}
              >
          ...
          {(search || filterModule !== 'all') && (
            <div className="flex items-center justify-between">
              <p className="text-xs text-slate-600">
                Showing <span className="text-slate-900 font-semibold">{displayed.length}</span> of {items.length} bookmarks
              </p>
              <button
          ...
          {filterModule === 'all' && !search && sortBy === 'module' ? (
                ...
                return (
                  <div key={modId} className="space-y-3">
                    <div className="flex items-center gap-2.5 pb-2 border-b border-slate-200">
                      ...
                      <span className="text-xs text-slate-600 ml-1">({modItems.length})</span>
            ...
            <div className="flex flex-col items-center py-16 text-center bg-slate-50 border border-slate-200 rounded-3xl">
              <Search className="size-8 text-slate-700 mb-3" />
              <p className="text-sm font-semibold text-slate-500">No results found</p>
              <p className="text-xs text-slate-600 mt-1">Try different keywords or clear your filters</p>
            </div>
          ) : (
            /* ── FLAT GRID / LIST ── */
          ...
          <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-200">
            {[
              ...
              <div key={stat.label} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2">
